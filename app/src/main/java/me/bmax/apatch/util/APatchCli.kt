...
/**
 * DSH-Folk：越狱（jailbreak/magica）安装已移除——DSH-Folk 不再打内核补丁，
 * 而是探测复用现有 root / Shizuku / 无线 ADB（见 dsh.PermissionManager）。
 * 保留此桩函数以兼容历史调用点，恒返回 false。
 */
fun installJailbreak(): Boolean {
    Log.i(TAG, "installJailbreak is a no-op in DSH-Folk (no kernel patching)")
    return false
}
