...
2. App 在本地把选中的会话、自己的设置与原生权限记录、以及（选了 vault 时）凭据补进这个包；
...
导入时会先探测包里有多少会话，然后问「跳过 / 直接恢复 / 停机恢复」—— 直接恢复是可行的（注册表
启动才读盘），只是写完之后若有任何工作区操作会被整份内存状态盖掉，所以推荐停机那一档。

「软件数据」= App 的 `SharedPreferences` + `audit/*.jsonl`，并且**不带密钥**：`webdav_*` 整组、
名字像 password/token/secret 的键、以及 `app_initialized`（带过去会让新设备跳过首次初始化）。
