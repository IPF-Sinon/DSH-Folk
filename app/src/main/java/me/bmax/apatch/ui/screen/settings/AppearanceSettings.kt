...
fun AppearanceSettingsContent(
    ...
    if (refreshThemeObserver) {
        ...
        currentStyle = prefs.getString("home_layout_style", "dsh")
    }
    ...
    val isListStyle = currentStyle != "dsh" && currentStyle != "kernelsu" && currentStyle != "focus" && !(isStatsLayout && statsTopLayout == "grid")
    // Focus布局样式（FocusUI），该样式下4个卡片支持独立壁纸
    val isFocusStyle = currentStyle == "focus"
    val isDashboardStyle = currentStyle == "dashboard_ui"
    // 默认ListUI布局（对应HomeScreen的else分支）；"dsh" 是 DSH-Folk 首页，独立于旧布局分支
    val isDefaultStyle = currentStyle !in listOf("dsh", "kernelsu", "focus", "circle", "dashboard_ui", "stats")
