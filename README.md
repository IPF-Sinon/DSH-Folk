...
**WebDAV 与插件共用一套配置。** 备份页的 WebDAV 客户端是 App 自己的（okhttp + 手写 PROPFIND），
只做一件朴素的事：导出后把整包 zip PUT 到 `<地址>/<路径>/`，并能列出、下载回来。而插件
（`dsh-config-manager`）另有一整套更硬的 WebDAV 同步（快照级增量、`index.json`、重定向跨源剥离
凭据、口令走 DSH 凭据不落文件），远端布局是 `<地址>/dsh-config-manager/<id>.json`。两者**分工**：
App 管自己的 zip，插件管 DSH 配置的增量同步。为了不必填两遍，App 的 WebDAV 配置框保存时会把
**地址 / 用户名 / 口令**三栏一并写进插件（`POST /sync/config`：url/username 落 `sync-config.json`、
口令落 DSH 凭据），打开时读回插件现状（`GET /sync/status`）作参照、并可一键填充地址与用户名。
几条边界：**路径不共用**（App 用它当 zip 子目录，插件在自己的 `dsh-config-manager/` 段下写，
同根不同子目录）；口令按插件设计**永不回传**，填充只填地址与用户名，口令仍要手输一次；App 本机
仍保留一份口令（它自己的 zip 上传要用）；DSH 没起来时保存照常成功、只提示插件侧未同步；插件当前
在 git 通道时，切到 WebDAV 前会先弹确认（git 配置保留、可切回）。
